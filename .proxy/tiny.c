...
int main(int argc, char **argv) {
  ...
  while (1) {
    ...
    // Getnameinfo를 호출하면서 hostname과 port가 채워진다.
    Getnameinfo((SA *)&clientaddr, clientlen, hostname, MAXLINE, port, MAXLINE,
                0);
    printf("Accepted connection from (%s, %s)\n", hostname, port); // 클라이언트 연결 성공 메세지
...
/* doit */
// 클라이언트의 요청 라인을 확인하고, 정적, 동적 콘텐츠인지 구분하고 서버로 전달
void doit(int fd)
{
  int is_static;
  struct stat sbuf;
  ...
  if (strcasecmp(method, "GET")) {  // method 스트링이 GET이 아니면 0이 아닌 값이 나옴
    ...
    "Tiny does not implement this method"); // 501 -> 알 수 없는 메소드를 사용하는 경우
    return;
  }
...
// -----------------------------------------
/* Client error
* 서버에 에러 처리 기능이 빠져 있어서 일부 오류에 대해서 체크하고 클라이언트에게 return
*/
...
만약 fork()의 반환값이 0 이라면, 즉 자식 프로세스라면 if 문을 수행한다.

fork()의 반환값이 0이 아니라면, 즉 부모 프로세스라면 if 문을 건너뛰고 Wait(NULL) 함수로 간다. 이 함수는 부모 프로세스가 먼저 도달해도 자식 프로세스가 종료될 때까지 기다리는 함수이다.

if 문 안에서 setenv 시스템 콜을 수행해 “QUERY_STRING”의 값을 cgiargs로 바꿔준다. 우선 순위가 1이므로 기존의 값과 상관없이 값이 변경된다.

dup2 함수를 실행해서 CGI 프로세스의 표준 출력을 fd(서버 연결 소켓 식별자)로 복사한다.
이제 STDOUT_FILENO의 값은 fd이다. 
다시 말해, CGI 프로세스에서 표준 출력을 하면 그게 서버 연결 식별자를 거쳐 클라이언트에 출력된다.
